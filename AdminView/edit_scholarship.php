...
$assessmentScheduleValue = '';
if (array_key_exists('assessment_schedule_at', $scholarshipOld)) {
    $assessmentScheduleValue = scholarshipOldValue($scholarshipOld, 'assessment_schedule_at');
} elseif (!empty($scholarshipData['assessment_schedule_at'])) {
    try {
        $assessmentScheduleValue = (new DateTime((string) $scholarshipData['assessment_schedule_at']))->format('Y-m-d\TH:i');
    } catch (Throwable $e) {
        $assessmentScheduleValue = '';
    }
}
$currentDateMin = date('Y-m-d');
$currentDateTimeMin = date('Y-m-d\TH:i');
$deadlineInputValue = scholarshipOldValue($scholarshipOld, 'deadline', (string) ($scholarshipData['deadline'] ?? ''));
$deadlineMinValue = ($deadlineInputValue !== '' && $deadlineInputValue < $currentDateMin)
    ? $deadlineInputValue
    : $currentDateMin;
$applicationOpenDateValue = scholarshipOldValue($scholarshipOld, 'application_open_date', (string) ($scholarshipData['application_open_date'] ?? ''));
$applicationOpenDateMinValue = ($applicationOpenDateValue !== '' && $applicationOpenDateValue < $currentDateMin)
    ? $applicationOpenDateValue
    : $currentDateMin;
$assessmentScheduleMinValue = ($assessmentScheduleValue !== '' && $assessmentScheduleValue < $currentDateTimeMin)
    ? $assessmentScheduleValue
    : $currentDateTimeMin;
...
$targetApplicantTypeValue = scholarshipOldValue($scholarshipOld, 'target_applicant_type', (string) ($scholarshipData['target_applicant_type'] ?? 'all'));
$preferredCourseValue = scholarshipOldValue($scholarshipOld, 'preferred_course', (string) ($scholarshipData['preferred_course'] ?? ''));
$preferredStrandValue = scholarshipOldValue($scholarshipOld, 'target_strand', (string) ($scholarshipData['target_strand'] ?? ''));
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-row-modern">
                                      ...
                                      <div class="form-group-modern">
                                        ...
                                        <input type="date" name="deadline" value="<?php echo htmlspecialchars($deadlineInputValue); ?>" min="<?php echo htmlspecialchars($deadlineMinValue); ?>" required>
                                    </div>
                                </div>
                                ...
                                <div class="form-row-modern">
                                    <div class="form-group-modern">
                                        ...
                                        <input type="date" name="application_open_date" value="<?php echo htmlspecialchars($applicationOpenDateValue); ?>" min="<?php echo htmlspecialchars($applicationOpenDateMinValue); ?>">
                                        <small class="helper-text">Optional: use this if the scholarship opens on a specific date.</small>
                                    </div>
                                    <div class="form-group-modern">
                                        <label><i class="fas fa-list-check"></i> Simple Application Process</label>
                                        <input type="text" name="application_process_label" maxlength="150" value="<?php echo htmlspecialchars(scholarshipOldValue($scholarshipOld, 'application_process_label', (string) ($scholarshipData['application_process_label'] ?? ''))); ?>" placeholder="e.g., Documents + Interview">
                                        <small class="helper-text">Keep this short so students can understand it quickly.</small>
                                    </div>
                                </div>
                                            ...
                                            <div class="form-group-modern" id="preferredCourseGroup">
                                                ...
                                                <select name="preferred_course" id="preferredCourseSelect">
                                                    <?php foreach ($preferredCourseOptions as $optionValue => $optionLabel): ?>
                                                        <option value="<?php echo htmlspecialchars($optionValue, ENT_QUOTES, 'UTF-8'); ?>" <?php echo scholarshipOldSelected($scholarshipOld, 'preferred_course', $optionValue, (string) ($scholarshipData['preferred_course'] ?? '')); ?>>
                                            ...
                                            <div class="form-group-modern" id="targetStrandGroup" style="<?php echo $showTargetStrandField ? '' : 'display: none;'; ?>">
                                                ...
                                                <select id="targetStrandInput" name="target_strand">
                                                    <?php foreach ($preferredStrandOptions as $optionValue => $optionLabel): ?>
                                                        <option value="<?php echo htmlspecialchars($optionValue, ENT_QUOTES, 'UTF-8'); ?>" <?php echo scholarshipOldSelected($scholarshipOld, 'target_strand', $optionValue, (string) ($scholarshipData['target_strand'] ?? '')); ?>>
                                                            <?php echo htmlspecialchars($optionLabel); ?>
                                                        </option>
                                                    <?php endforeach; ?>
                                                </select>
                                ...
                                <div class="form-group-modern">
                                    ...
                                    <input type="datetime-local" name="assessment_schedule_at" value="<?php echo htmlspecialchars($assessmentScheduleValue); ?>" min="<?php echo htmlspecialchars($assessmentScheduleMinValue); ?>">
                                ...
                                <div class="form-group-modern">
                                    ...
                                    <textarea name="assessment_details" rows="3" placeholder="Instructions or reminders that should appear for all accepted applicants"><?php echo htmlspecialchars(scholarshipOldValue($scholarshipOld, 'assessment_details', (string) ($scholarshipData['assessment_details'] ?? ''))); ?></textarea>
                                ...
                                <div class="form-card-modern" style="margin-top: 24px; border: 1px solid var(--gray-200); box-shadow: none;">
                                    ...
                                    <div class="card-body">
                                        <div class="form-group-modern">
                                            ...
                                            <textarea name="post_application_steps" rows="3" placeholder="Example: After submission, the provider reviews documents, shortlists applicants, then releases the final decision."><?php echo htmlspecialchars(scholarshipOldValue($scholarshipOld, 'post_application_steps', (string) ($scholarshipData['post_application_steps'] ?? ''))); ?></textarea>
                                            <small class="helper-text">Explain the next stage in simple language so students know what to expect.</small>
                                        </div>

                                        <div class="form-row-modern">
                                            <div class="form-group-modern">
                                                <label><i class="fas fa-rotate"></i> Renewal Conditions</label>
                                                <textarea name="renewal_conditions" rows="3" placeholder="Example: Maintain a 2.00 GWA, no failing grades, and renew every semester."><?php echo htmlspecialchars(scholarshipOldValue($scholarshipOld, 'renewal_conditions', (string) ($scholarshipData['renewal_conditions'] ?? ''))); ?></textarea>
                                            </div>
                                            <div class="form-group-modern">
                                                <label><i class="fas fa-triangle-exclamation"></i> Restrictions / Obligations</label>
                                                <textarea name="scholarship_restrictions" rows="3" placeholder="Example: Full-time enrollment only, selected campuses only, or return-service obligation."><?php echo htmlspecialchars(scholarshipOldValue($scholarshipOld, 'scholarship_restrictions', (string) ($scholarshipData['scholarship_restrictions'] ?? ''))); ?></textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
