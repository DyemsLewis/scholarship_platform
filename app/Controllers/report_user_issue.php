...
require_once __DIR__ . '/../Models/UserIssueReport.php';
...
function redirectToIssueTarget(string $type, string $title, string $message, bool $appendFragment = false): void
{
    ...
    header('Location: ' . normalizeAppUrl(normalizeUserIssueRedirectUrl($_POST['redirect_url'] ?? null, $appendFragment)));
    exit();
}
...
function redirectToProfileWithIssueErrors(array $errors, array $oldInput): void
{
    ...
    header('Location: ' . normalizeAppUrl(normalizeUserIssueRedirectUrl($_POST['redirect_url'] ?? null, true)));
    exit();
}
