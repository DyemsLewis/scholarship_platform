...
function denyScholarshipAction(string $message): void
{
    ...
    header('Location: ../AdminView/manage_scholarships.php');
    exit();
}
...
function scholarshipReviewRedirect(string $target, int $scholarshipId = 0): void
{
    if ($target === 'edit' && $scholarshipId > 0) {
        header('Location: ' . buildEntityUrl('../AdminView/edit_scholarship.php', 'scholarship', $scholarshipId, 'edit', ['id' => $scholarshipId]));
        exit();
    }

    header('Location: ../AdminView/scholarship_reviews.php');
    exit();
}
