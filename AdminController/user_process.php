...
function denyUserManagementAction(string $message): void {
    ...
    if (in_array($redirectTarget, ['provider_review', 'provider_reviews'], true)) {
        header('Location: ../AdminView/provider_reviews.php');
        exit();
    }
    header('Location: ../AdminView/manage_users.php');
    exit();
}
...
function redirectBackToEditUser(int $userId, array $errors = [], ?string $errorMessage = null, array $oldInput = []): void {
    ...
    header('Location: ' . buildEntityUrl('../AdminView/edit_user.php', 'user', $userId, 'edit', ['id' => $userId]));
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'update_status') {
        ...
        header('Location: ' . $redirectAfterAction);
        exit();
    }
